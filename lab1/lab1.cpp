...
void runLab1() {
    ...
    switch (choice) {
        case 1: {
            ...
            else intNumber &= ~(1 << (sizeof(int)*8 - bitPosition));
        ...
        case 2: {
            // ПЕРЕИНИЦИАЛИЗИРУЕМ union с текущим числом
            union { float floatNumber; unsigned int floatValue; };
            if (newBit==1) floatValue |= (1U << (sizeof(float)*8 - bitPosition));
            else floatValue &= ~(1U << (sizeof(float)*8 - bitPosition));
        ...
        case 3: {
            ...
            // ПЕРЕИНИЦИАЛИЗИРУЕМ union с текущим числом
            union { double doubleNumber; unsigned long long doubleValue; };
